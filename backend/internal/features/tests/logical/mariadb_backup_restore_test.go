...
func Test_BackupAndRestoreMariadb_WithExcludeTables_ExcludedTablesNotRestored(t *testing.T) {
	env := config.GetEnv()
	cases := []struct {
		name    string
		version tools.MariadbVersion
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			testMariadbBackupRestoreWithExcludeTablesForVersion(t, tc.version, tc.port)
		})
	}
}
