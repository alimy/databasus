...
func openTestConn(t *testing.T, port string) *pgx.Conn {
	...
	dsn := fmt.Sprintf(
		...
		config.GetEnv().TestLocalhost, portInt,
	)
...
func Test_Validate_RejectsVersionBelow17(t *testing.T) {
	m := validModel()
	m.Version = tools.PostgresqlVersion16

	err := m.Validate()
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "17 or 18")
}

func Test_Validate_AcceptsVersion17(t *testing.T) {
	m := validModel()
	m.Version = tools.PostgresqlVersion17

	assert.NoError(t, m.Validate())
}

func Test_Validate_AcceptsVersion18(t *testing.T) {
	m := validModel()
	m.Version = tools.PostgresqlVersion18
...
func Test_TestReplicationConnection_FailsForFullIncrementalWhenSummarizeWalOff(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			...
			require.Error(t, err)
			assert.Contains(t, err.Error(), "summarize_wal")
		})
	}
}
...
func Test_TestReplicationConnection_FailsForWrongPassword(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			...
			err := m.TestReplicationConnection(testLogger(), nil)
			require.Error(t, err)
			assert.Contains(t, err.Error(), "credentials wrong")
		})
	}
}
...
func Test_CreateReplicationOnlyUser_NewUserCanOpenReplicationConnection(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			...
			dsn := fmt.Sprintf(
				"host=%s port=%d user=%s password=%s dbname=postgres sslmode=disable replication=database",
...
func Test_TestReplicationConnection_OnSimulatedRds_FixMessageMentionsParameterGroup(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			if fx.noSummaryPort() == "" {
				t.Fatalf("%s no-summary port not configured", fx.name)
			...
			err := m.TestReplicationConnection(testLogger(), nil)
			require.Error(t, err)
			assert.Contains(t, err.Error(), "parameter group")
		})
	}
}

func Test_TestReplicationConnection_OnSimulatedAzure_FixMessageMentionsServerParameters(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			if fx.noSummaryPort() == "" {
				t.Fatalf("%s no-summary port not configured", fx.name)
			...
			err := m.TestReplicationConnection(testLogger(), nil)
			require.Error(t, err)
			assert.Contains(t, err.Error(), "Server parameters")
		})
	}
}

func Test_TestReplicationConnection_OnSimulatedGcp_FixMessageMentionsGcloud(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			if fx.noSummaryPort() == "" {
				t.Fatalf("%s no-summary port not configured", fx.name)
...
func Test_TestReplicationConnection_LeavesNoProbeSlotsBehind(t *testing.T) {
	for _, fx := range physicalFixtures() {
		t.Run(fx.name, func(t *testing.T) {
			...
			m := newTestModel(t, fx.port())
			require.NoError(t, m.TestReplicationConnection(testLogger(), nil))

			adminConn := openTestConn(t, fx.port())

			var count int
			require.NoError(t, adminConn.QueryRow(t.Context(),
				"SELECT count(*) FROM pg_replication_slots WHERE slot_name LIKE 'databasus_probe_%'",
			).Scan(&count))
