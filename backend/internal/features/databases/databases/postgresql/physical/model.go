...
func (p *PostgresqlPhysicalDatabase) Validate() error {
	...
	if p.Version != tools.PostgresqlVersion17 && p.Version != tools.PostgresqlVersion18 {
		return errors.New("physical backup requires PostgreSQL 17 or 18")
	}
...
func (p *PostgresqlPhysicalDatabase) TestReplicationConnection(
	...
	replConn, err := openReplicationConn(ctx, p, encryptor)
	if err != nil {
		return classifyReplicationConnectError(err)
	}
	defer closeConnQuietly(ctx, replConn, logger)

	// Re-open an ordinary connection for platform detection and the slot
	// inspection query (pg_replication_slots is not in the replication
	// command set, so we can't run it on the replication conn).
	conn, err := openConn(ctx, p, encryptor)
	if err != nil {
	...
	if settings.walLevel != "replica" && settings.walLevel != "logical" {
		return errors.New(
			fixMessage(
				platform,
				fmt.Sprintf(
					"wal_level must be 'replica' or 'logical' (currently %q)",
					settings.walLevel,
				),
				"set wal_level via ALTER SYSTEM SET wal_level = replica; restart PostgreSQL",
				"parameter group",
				"Server parameters → wal_level",
				"--database-flags=wal_level=replica",
			),
		)
	}
	...
	if settings.maxWalSenders == 0 {
		return errors.New(
			fixMessage(
				...
				"set max_wal_senders via ALTER SYSTEM and restart PostgreSQL",
				"parameter group",
				"Server parameters → max_wal_senders",
				"--database-flags=max_wal_senders=10",
			),
		)
	}
	...
	if settings.maxReplicationSlots == 0 {
		return errors.New(
			fixMessage(
				...
				"set max_replication_slots via ALTER SYSTEM and restart PostgreSQL",
				"parameter group",
				"Server parameters → max_replication_slots",
				"--database-flags=max_replication_slots=10",
			),
		)
	}
	...
	if p.SystemIdentifier != nil {
		...
		if currentSysID != *p.SystemIdentifier {
			return fmt.Errorf(
				"different cluster — refused: expected system_identifier %s, got %s",
				*p.SystemIdentifier,
				currentSysID,
			)
		}
	...
	if err := p.probeSlotPermissions(ctx, logger, encryptor); err != nil {
		return err
	}
...
// slot to verify the connecting role has REPLICATION privilege at the SQL
// surface. Used inside TestReplicationConnection as a final sanity check.
func (p *PostgresqlPhysicalDatabase) probeSlotPermissions(
	ctx context.Context,
	logger *slog.Logger,
	encryptor encryption.FieldEncryptor,
) error {
	...
	probeName := "databasus_probe_" + strings.ReplaceAll(uuid.New().String(), "-", "")

	if _, err := conn.Exec(ctx, "SELECT pg_create_physical_replication_slot($1)", probeName); err != nil {
		return fmt.Errorf(
			"cannot create replication slot (REPLICATION privilege likely missing): %w",
			err,
		)
	}

	if _, err := conn.Exec(ctx, "SELECT pg_drop_replication_slot($1)", probeName); err != nil {
		logger.Warn("probe slot leaked; manual cleanup required",
			"slot_name", probeName, "error", err)

		return fmt.Errorf(
			"cannot drop replication slot (REPLICATION privilege likely limited): %w",
			err,
		)
	}
...
func readReplicationSettings(ctx context.Context, conn *pgx.Conn) (*replicationSettings, error) {
	...
	`).Scan(&s.walLevel, &s.summarizeWal, &s.maxWalSenders, &s.maxReplicationSlots); err != nil {
...
func fixMessage(plat platform, headline, selfFix, rdsFix, azureFix, gcpFix string) string {
	switch plat {
	case platformRds:
		return fmt.Sprintf("%s; on AWS RDS: %s", headline, rdsFix)
	case platformAzure:
		return fmt.Sprintf("%s; on Azure: %s", headline, azureFix)
	case platformGcp:
		return fmt.Sprintf("%s; on GCP: gcloud sql instances patch <instance> %s", headline, gcpFix)
	case platformUnknownManaged:
		return fmt.Sprintf("%s; check your managed PG platform docs for how to change this", headline)
	default:
		return fmt.Sprintf("%s; %s", headline, selfFix)
	}
}

func classifyReplicationConnectError(err error) error {
	msg := err.Error()

	switch {
	case strings.Contains(msg, "no pg_hba.conf entry"):
		return fmt.Errorf(
			"replication connect failed: no pg_hba.conf entry — self-managed: add `host replication <user> <cidr> scram-sha-256`; managed PG: enable external replication in the platform console: %w",
			err,
		)
	case strings.Contains(msg, "password authentication failed"):
		return fmt.Errorf("replication connect failed: credentials wrong: %w", err)
	case strings.Contains(msg, "must have replication privilege") ||
		...
		return fmt.Errorf(
			"replication connect failed: user lacks REPLICATION — self-managed: GRANT REPLICATION; RDS: GRANT rds_replication; Azure/GCP: enable in console: %w",
			err,
		)
	default:
...
// conninfo so IDENTIFY_SYSTEM, CREATE_REPLICATION_SLOT, etc. become available.
func openReplicationConn(
	ctx context.Context,
	p *PostgresqlPhysicalDatabase,
	encryptor encryption.FieldEncryptor,
) (*pgx.Conn, error) {
