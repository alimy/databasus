...
func Test_CheckDeadNodesAndFailBackups_WhenNodeDies_FailsBackupAndCleansUpRegistry(t *testing.T) {
	...
	defer func() {
		...
		if mockNodeID != uuid.Nil {
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: mockNodeID})
		}
		cache_utils.ClearAllCache()
	}()
	...
	assert.NoError(t, err)
	assert.Len(t, backups, 1)
	assert.Equal(t, backups_core_logical.BackupStatusFailed, backups[0].Status)
...
func Test_CalculateLeastBusyNode_SelectsNodeWithBestScore(t *testing.T) {
	t.Run("Nodes with same throughput", func(t *testing.T) {
		...
		defer func() {
			...
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: node1ID})
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: node2ID})
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: node3ID})
			cache_utils.ClearAllCache()
		}()
	...
	t.Run("Nodes with different throughput", func(t *testing.T) {
		...
		defer func() {
			// Clean up all mock nodes
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: node100MBsID})
			backupNodesRegistry.UnregisterNodeFromRegistry(nodes.BackupNode{ID: node50MBsID})
			cache_utils.ClearAllCache()
		}()
...
func Test_StartBackup_WhenBackupCompletes_DecrementsActiveTaskCount(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Len(t, backups, 1)
	assert.Equal(t, backups_core_logical.BackupStatusCompleted, backups[0].Status)
...
func Test_StartBackup_WhenCloudAndSubscriptionExpired_CreatesFailedBackup(t *testing.T) {
	...
	assert.Equal(t, backups_core_logical.BackupStatusFailed, newestBackup.Status)
...
func Test_StartBackup_WhenCloudAndSubscriptionActive_ProceedsNormally(t *testing.T) {
	...
	newestBackup := backups[0]
	assert.Equal(t, backups_core_logical.BackupStatusCompleted, newestBackup.Status)
