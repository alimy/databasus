...
type TimelineDecision struct {
	...
	ActualSysID   string // TimelineDifferentCluster — live cluster value
}
...
func UploadHistoryFile(
	...
	sidecarFilename := storageObjectName + ".metadata"
	...
	sidecar := physical_dto.PhysicalWalHistoryMetadata{
		...
		Encryption:          encryptionEnumFromConfig(encryption),
...
func buildHistoryArtifactReader(
	...
	if encryption != backups_config_logical.BackupEncryptionEncrypted {
...
func encryptionEnumFromConfig(enc backups_config_logical.BackupEncryption) physical_enums.PhysicalBackupEncryption {
	if enc == backups_config_logical.BackupEncryptionEncrypted {
		return physical_enums.PhysicalBackupEncryptionAes256Gcm
	}

	return physical_enums.PhysicalBackupEncryptionNone
}
