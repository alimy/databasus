...
// Reused by WithBackupSlot, RunStartupCleanup, and the OnBeforeDatabaseRemove
// hook (PR 3) so all three drop the same slot without persisting any name
// in our DB.
//
// PG identifier rules: lowercase letters, digits, underscores only. UUID
// hyphens are stripped; we keep all 32 hex chars so the name is reversibly
// mappable to the database row.
...
// dropBackupSlotIfExists drops the named slot, treating "not found" as
// success. Used by both WithBackupSlot (drop-if-exists prelude + post-defer)
// and RunStartupCleanup.
