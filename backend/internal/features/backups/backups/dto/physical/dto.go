...
// page can be all one type). Type/Status are optional; an unknown value simply
// matches nothing. BeforeDate keeps only backups created strictly before it.
type GetPhysicalBackupsRequest struct {
	Limit      int                                `form:"limit"`
	Offset     int                                `form:"offset"`
	Type       *physical_enums.PhysicalBackupType `form:"type"`
