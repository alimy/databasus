...
// later, restore verification) consumes: it opens each stored artifact, decrypts
// and decompresses it on the fly, and re-tars it into a
// ready-to-pg_combinebackup layout — full/ , incr-N/ (each with backup_manifest),
// wal/<segment|history>, plus a trailing MANIFEST.sha256. The reconstruction
// command (pg_combinebackup + recovery settings) is shown in the UI, not shipped
// in the tar. It is transport- and auth-agnostic: callers decide who may stream
...
func (rw *Writer) writeWalSegment(
	...
	// PG's restore_command fetches by the bare WAL name; the storage object name
	// (<db>-WAL-tl<TL>-<file>.zst) must not leak into wal/.
	return streamTarEntry(tarWriter, "wal/"+segment.WalFilename, 0o600, reader, checksums)
}
